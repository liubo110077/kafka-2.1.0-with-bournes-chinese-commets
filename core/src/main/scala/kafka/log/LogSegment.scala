...
/**
 * 可以看做是对一组日志和索引文件数据的封装
 * 每个Segment有两个组件：一个log和一个index。log是FileRecords的实例，包含了真正的消息。index是OffsetIndex的实例，映射了offset和message的物理位置。
 * 每一个segment有一个base offset，小于等于segment中所有消息的最小的offset，大于前一个segment中最大的offset。
 ...
 * @param baseOffset A lower bound on the offsets in this segment。每个 LogSegment 都有一个基准偏移量 baseOffset，用来表示当前 LogSegment中第一条消息的offset。
...
class LogSegment private[log] (val log: FileRecords,
                               ...
                               val baseOffset: Long, //每个日志段对象保存自己的起始位移,baseOffset--这是非常重要的属性！事实上，你在磁盘上看到的文件名就是 baseOffset 的值。每个 LogSegment 对象实例一旦被创建，它的起始位移就是固定的了，不能再被更改。
                               val indexIntervalBytes: Int, //indexIntervalBytes 值其实就是 Broker 端参数 log.index.interval.bytes 值，它控制了日志段对象新增索引项的频率。默认情况下，日志段至少新写入 4KB 的消息数据才会新增一条索引项。
                               val rollJitterMs: Long, //而 rollJitterMs 是日志段对象新增倒计时的“扰动值”。因为目前 Broker 端日志段新增倒计时是全局设置，这就是说，在未来的某个时刻可能同时创建多个日志段对象，这将极大地增加物理磁盘 I/O 压力。有了 rollJitterMs 值的干扰，每个新增日志段在创建时会彼此岔开一小段时间，这样可以缓解物理磁盘的 I/O 负载瓶颈。
   ...
   */
  @nonthreadsafe
  def append(largestOffset: Long,  // 待追加消息的起始 offset
             largestTimestamp: Long,  // 待追加消息中的最大 offset
             shallowOffsetOfMaxTimestamp: Long,   // 最大时间戳消息对应的 offset
             records: MemoryRecords): Unit = {   // 待追加的消息数据
