...
class LogManager(logDirs: Seq[File],
  ...
  private def createAndValidateLogDirs(dirs: Seq[File], initialOfflineDirs: Seq[File]): ConcurrentLinkedQueue[File] = {
    ...
    //遍历目录 server.properties的log.dirs=/data0/log , /data1/log , /data2/log
    //为了提升磁盘读取性能，可以配置log.dirs为多个磁盘
    for (dir <- dirs) {
      try {
        if (initialOfflineDirs.contains(dir))
          throw new IOException(s"Failed to load ${dir.getAbsolutePath} during broker startup")

        if (!dir.exists) { //目录不存在，自动创建目录
  ...
  private def loadLogs(): Unit = {
    ...
    for (dir <- liveLogDirs) {
      try {
        //numRecoveryThreadsPerDataDir默认值是1
  ...
  def startup() {
    ...
    /**
     * todo 内部实现是ScheduledThreadPoolExecutor
     *
     */
    if (scheduler != null) {
       ...
       * TODO 表示已经刷写到磁盘的offset信息，对应LEO信息。
       ...
       * Kafka在启动时会检查文件的完整性，如果没有.kafka_cleanshutdown这个文件，就会进入一个recover逻辑，recover就是从此文件中的offset开始。
       */
      scheduler.schedule("kafka-recovery-point-checkpoint",
                         ...
                         TimeUnit.MILLISECONDS)
      //todo 那么 replication-offset-checkpoint：用来存储每个replica的HW，表示已经被commited的offset信息。失败的follower开始恢复时，会首先将自己的日志截断到上次的checkpointed时刻的HW，然后向leader拉取消息。kafka有一个定时任务负责将所有分区的HW刷写到复制点文件replication-offset-checkpoint中，定时周期由broker端参数replica.high.watermark.checkpoint.interval.ms配置，默认值5000，即5s。
      // 高水位怎么没有线程来维护呢？？我理解高水位的更新得在replica副本向leader副本发起拉取数据的时候来更新。
      /**
       * log-start-offset-checkpoint：对应logStartOffset，用来标识日志的起始偏移量。
       * kafka中有一个定时任务负责将所有分区的logStartOffset刷写到起始点文件log-start-offset-checkpoint中，
       * 定时周期有broker端参数log.flush.start.offset.checkpoint.interval.ms配置，默认值60000，即60s,
       * 正常情况下，kafka会每过1分钟更新一次，用最小segment的最小offset更新。
       */
      ...
      /**
       * todo 问题： 我们执行delete命令后，看到topic并没有立即被删除，只是打了个deleted的标记。那么kafka是怎么清理这些被打了deleted的topic的。
       *  答： 从源码部分，大概知道，流程应该是在一个fileDeleteDelayMs的周期里边，kafka会先删除数据，在情况所有数据都被删除后（logsToBeDeleted.isEmpty），在下一个fileDeleteDelayMs周期，就会调用removedLog.delete()方法把partition删除。达到删除topic的目的。
       *
       */
  ...
  def cleanupLogs() {
    ...
    val deletableLogs = {
      if (cleaner != null) {
        ...
        //如果logCleaner为空，就不能删除compact的Log？？？
        currentLogs.filter {
          //todo 如果日志被compact了，就过滤掉，为啥啊？？？
  ...
  private def flushDirtyLogs(): Unit = {
    ...
    for ((topicPartition, log) <- currentLogs.toList ++ futureLogs.toList) {
      try {
        ...
        //超过刷盘周期间隔，建议不要设置，可以利用操作系统的flush能力，能够提高效率，默认值是Long.MaxValue
...
object LogManager {
  //表示已经刷写到磁盘的记录。recoveryPoint以下的数据都是已经刷到磁盘上的了。
  val RecoveryPointCheckpointFile = "recovery-point-offset-checkpoint"
  //用来存储每个replica的HighWatermark的(high watermark (HW)，表示已经被commited的message，HW以下的数据都是各个replicas间同步的，一致的。
  ...
  def apply(config: KafkaConfig,
    ...
    /*
      ...
      "file.delete.delay.ms" -> {Long@3384} 60000
      "max.message.bytes" -> {Integer@3386} 1000012
      "min.compaction.lag.ms" -> {Long@3388} 0
      "message.timestamp.type" -> "CreateTime"
      "min.insync.replicas" -> {Integer@3392} 1
      "message.downconversion.enable" -> {Boolean@3394} true
      "segment.jitter.ms" -> {Long@3388} 0
      "preallocate" -> {Boolean@3397} false
      "index.interval.bytes" -> {Integer@3399} 4096
      "min.cleanable.dirty.ratio" -> {Double@3401} 0.5
      "unclean.leader.election.enable" -> {Boolean@3397} false
      "retention.bytes" -> {Long@3404} -1
      "delete.retention.ms" -> {Long@3406} 86400000
      "cleanup.policy" -> {Arrays$ArrayList@3408}  size = 1
      "flush.ms" -> {Long@3410} 9223372036854775807
      "segment.bytes" -> {Integer@3412} 1073741824
      "segment.ms" -> {Long@3414} 604800000
      "retention.ms" -> {Long@3416} 604800000
      "message.timestamp.difference.max.ms" -> {Long@3418} 9223372036854775807
      "segment.index.bytes" -> {Integer@3420} 10485760
      "flush.messages" -> {Long@3422} 9223372036854775807
     */
    ...
    //todo 创建LogManager对象
    new LogManager(logDirs = config.logDirs.map(new File(_).getAbsoluteFile), //log文件的目录
