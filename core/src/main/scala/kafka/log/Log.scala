 ...
 */
@threadsafe
class Log(@volatile var dir: File,  //日志所在的文件夹路径，也就是主题分区的路径
          @volatile var config: LogConfig,
          @volatile var logStartOffset: Long,   //当前日志最早的位移值
          @volatile var recoveryPoint: Long,    //日志的恢复点，LEO的信息
          scheduler: Scheduler,
   ...
   * todo
   *    记录当前高水位，为了确保那些包含等于或高于当前高水位的offset的日志段不能够删除。
   *    这意味着active的日志段只能在高水位等于LEO（这种情况永远不可能在有持续负载的分区发生）的情况下才能够被删除。
   *    这样可以防止log start offset超过高水位
   */
  ...
  private def initializeLeaderEpochCache(): LeaderEpochFileCache = {
    ...
    //创建 leader-epoch-checkpoint 文件,此文件是用于记录每个当前Kafka Leader 的epoch，用于一致性
    val checkpointFile = new LeaderEpochCheckpointFile(LeaderEpochFile.newFile(dir), logDirFailureChannel)
    //实例化 LeaderEpochFileCache 对象，此对象用来缓存 tp 和 leo 的关系
  ...
  private def append(records: MemoryRecords, isFromClient: Boolean, assignOffsets: Boolean, leaderEpoch: Int): LogAppendInfo = {
    maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
      ...
      // 如果压根就不需要写入任何消息，直接返回即可(也就是说在第1步中没有发现有message 这个这里 shallowCount 是0)
      if (appendInfo.shallowCount == 0)
        return appendInfo

      // trim any invalid bytes or partial messages before appending it to the on-disk log
      //  消息格式规整，即删除无效格式消息或无效字节，根据步骤 1 方法返回的LogAppendInfo 对象，将未通过验证的消息截断
  ...
  private def deleteSegments(deletable: Iterable[LogSegment]): Int = {
    maybeHandleIOException(s"Error while deleting segments for $topicPartition in dir ${dir.getParent}") {
      ...
      if (numToDelete > 0) {
        ...
        //如果一个Log的所有segment都可以被删除，先创建一个新的segment，然后删掉前面N个
        if (segments.size == numToDelete)
          roll()
        lock synchronized {
          //Close file handlers used by log but don't write to disk. This is called if the log directory is offline
          //如果被log使用的file handler已经被关闭，抛出异常
          checkIfMemoryMappedBufferClosed()
  ...
  private def deletableSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean): Iterable[LogSegment] = {
    if (segments.isEmpty || replicaHighWatermark.isEmpty) {
      ...
      //可删除的LogSegment，初始值为空
      val deletable = ArrayBuffer.empty[LogSegment]
      //segments的底层实现是基于跳表的ConcurrentNavigableMap[java.lang.Long, LogSegment]，有序
      ...
      while (segmentEntry != null) {
        ...
        /*
          todo
            2.按照日志大小来删除日志
            size: The size of the log in bytes including all the log segments in this log ordered from oldest to newest
                假设，test-topic-0 一共有3个segment，大小一共为9MB，每个为3MB
                其首先计算日志文件的总大小size和retentionSize的差值diff，即计算需要删除的日志总大小，
                然后从日志文件中的第一个日志分段开始进行查找可删除的日志分段的文件集合deletableSegments。
                查找出deletableSegments之后就执行删除操作，这个删除操作和基于时间的保留策略的删除操作相同，这里不再赘述。
                retentionSize为8MB
                var diff = size - config.retentionSize
                def shouldDelete(segment: LogSegment, nextSegmentOpt: Option[LogSegment]) = {
                  if (diff - segment.size >= 0) {  1MB减去第一个segment的大小3MB = -2MB，小于0，此时不删除，只有等Log的大小-8MB大于 3MB的时候才会将第一个segment加入要删除的segment
                    diff -= segment.size
                    true
                  } else {
                    false
         ...
         */
        //必须确保高水位大于当前segment的最大offset，
        //必须还存在一个segment，否则没有地方来追加数据
  ...
  def deleteOldSegments(): Int = {
    if (config.delete) {
      /**
       * todo
       *  1. deleteRetentionMsBreachedSegments 删除保留时间超时的segment
       *    retention.ms
       *    This configuration controls the maximum time we will retain a log before we will discard old log segments to free up space if we are using the
       *    "delete" retention policy. This represents an SLA on how soon consumers must read their data. If set to -1, no time limit is applied.
       *  2. deleteRetentionSizeBreachedSegments 删除大小超过大小的segment
       *    retention.bytes
       ...
       *  3. deleteLogStartOffsetBreachedSegments 删除log的start offset超过的segment
       */
  ...
  private def deleteRetentionSizeBreachedSegments(): Int = {
    ...
    //如果当前Log的大小小于retention的大小，直接返回0，不用删除
    if (config.retentionSize < 0 || size < config.retentionSize) return 0
    /**
     * 如果当前最新的日志大小加上下一个即将删除的 segment 分段的大小超过阈值，那么就允许删除该 segment，否则就不允许。
     *
     * size: The size of the log in bytes including all the log segments in this log ordered from oldest to newest
     */
    var diff = size - config.retentionSize
   ...
   * 假设logStartOffset等于25，日志分段1的起始偏移量为0，日志分段2的起始偏移量为11，日志分段3的起始偏移为23，那么我们通过如下动作收集可删除的日志分段的文件集合deletableSegments：
  ...
  private def deleteSegment(segment: LogSegment) {
    ...
    lock synchronized {
      //从Log的的segments map中删除segment，segment以后就不能被使用了
