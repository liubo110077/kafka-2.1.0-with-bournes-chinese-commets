...
public class KafkaChannel {
    ...
    public NetworkReceive read() throws IOException {
        ...
        //读取channel中的数据到NetworkReceive中的size和buffer中
        receive(receive);
        //如果已经读取完了一个完整的消息，解决拆包粘包问题
        //size和buffer都已经满了，就表明消息读完了
        if (receive.complete()) {
            //rewind buffer,将position置为0，可以重新读取其中的数据了
            receive.payload().rewind();
            //如果已经读取完毕，将result设置为receive，外层循环会一直运行
            result = receive;
            //将receive置为空，下次调用read方法的时候，就会重新初始化NetworkReceive
            receive = null;
    ...
    public Send write() throws IOException {
        //之前已经在Selector的send方法里，已经调用过channel.setSend(send);
        Send result = null;
    ...
    private boolean send(Send send) throws IOException {
        ...
        //真正发送请求long written = channel.write(buffers);
