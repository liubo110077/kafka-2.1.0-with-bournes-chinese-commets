...
final class ClusterConnectionStates {
    ...
    public boolean canConnect(String id, long now) {
        //第一次nodeState没有数据，直接返回true
        //返回null表明主机没有建立过连接
        NodeConnectionState state = nodeState.get(id);
        if (state == null)
            return true;
        else
            return state.state.isDisconnected() &&  //虽然之前有连接，但是已经断开了
                   now - state.lastConnectAttemptMs >= state.reconnectBackoffMs; //now-state.lastConnectAttemptMs是等了多长时间了，如果等待时间已经大于等于连接重试的超时时间
    }
    ...
    public void connecting(String id, long now, String host, ClientDnsLookup clientDnsLookup) throws UnknownHostException {
        //如果nodeState中包含了node id，还未创建连接时不包含
        if (nodeState.containsKey(id)) {
            ...
            //noteState中，记录node id，并将状态改为ConnectionState.CONNECTING
    ...
    public boolean isReady(String id, long now) {
        //第一次nodeState.get(id)为空，所以isReady一定返回false
