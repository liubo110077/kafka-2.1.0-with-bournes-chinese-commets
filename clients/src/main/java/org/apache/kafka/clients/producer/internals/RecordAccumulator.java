...
public final class RecordAccumulator {
    ...
    //分区-> Deque. Deque中存储的是ProducerBatch
    //把消息封装成一个一个的批次，默认大小16KB
    //map的key是TopicPartition，而value是Deque<ProducerBatch>
    //map的底层实现是new CopyOnWriteMap<>()
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            /**
             * TODO 步骤1，根据TopicPartition获取或者创建队列
             */
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {//TODO 加锁粒度尽量小，根据每一个TopicPartition来加锁
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                //TODO 步骤2，尝试将消息添加到队列当中去，队列初始的size为0，没有添加成功
            ...
            /**
             * batch.size 默认16KB
             * AbstractRecords.estimateSizeInBytesUpperBound：估算消息的大小，并没有考虑压缩算法，但是我发现代码中为了估算消息的大小，竟然new了一个ByteBuffer对象，感觉有优化的空间啊
             * 最后取出消息估算大小以及16KB之间的最大值，如果消息是1MB，那么就返回1MB
             * 如果消息不大于16KB，那么申请的内存为16KB，这样做是为了BufferPool中重用Buffer
             * 如果消息大小大于16KB，那么batch中只有一条记录了，而且BufferPool中也无法重用Buffer，因为ByteBuffer的创建十分消耗内存，所以一定要尽量的将消息的大小控制在16KB以内
             * 如果消息确实有很多要大于16KB,那么要考虑将batch.size的大小调大，大于message的大小，这样才能充分利用ByteBuffer的内存池的优势
             */
            ...
            /**
             * TODO 步骤4，根据批次大小分配内存
             *   BufferPool Buffer池，默认大小为32MB
             *
             */

            buffer = free.allocate(size, maxTimeToBlock);
            synchronized (dq) {//加的分段锁，如果有多个线程同时向同一个topic的分区内添加message，同时只有一个线程能够获得锁，其他线程排队，第一个线程执行步骤5失败，然后继续执行步骤6，7，之后topic的deque中就会存在。第二个线程恢复运行后，就会成功的执行步骤5，然后返回。
                // Need to check if producer is closed again after grabbing the dequeue lock.
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                //TODO 步骤5，再次尝试将消息添加到队列中，队列初始的size为0，没有添加成功
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, headers, callback, dq);
                if (appendResult != null) {
                    // Somebody else found us a batch, return the one we waited for! Hopefully this doesn't happen often...
                    //注意，如果代码能够运行到这里，表明不需要allocate buffer，会在finally中将buffer回收
                    return appendResult;
                }

                /**
                 * TODO 步骤6，创建一个ProducerBatch，尝试把消息添加到batch中
                 *
                 *
                 */
                ...
                /**
                 * TODO 步骤7，然后将消息添加到队列中：dq.addLast(batch);
                 */
                dq.addLast(batch);
    ...
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers,
        ...
        //deque队列中初始没有元素，所以会返回null
        ProducerBatch last = deque.peekLast();
        if (last != null) {
            FutureRecordMetadata future = last.tryAppend(timestamp, key, value, headers, callback, time.milliseconds());
            if (future == null)//batch中没有地方了
                last.closeForRecordAppends();
            else //成功的在batch中添加了message
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
         ...
         *   如果有等待内存释放的this.waiters.size()>0，说明当前的RecordAccumulator内存已经被耗尽
         */
         ...
         */
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            TopicPartition part = entry.getKey();//TopicPartition
            Deque<ProducerBatch> deque = entry.getValue();//TopicPartition对应的队列

            Node leader = cluster.leaderFor(part);//获取TopicPartition的Leader节点
            synchronized (deque) {
                if (leader == null && !deque.isEmpty()) {
                    // This is a partition for which leader is not known, but messages are available to send.
                    // Note that entries are currently not removed from batches when deque is empty.
                    // 没有货的leader的topic会被加入unknownLeaderTopics，Sender的sendProducerData方法，也就是caller会重新更新metadata
                    ...
                    if (batch != null) {
                        /**
                         *  等待时间
                         *  Math.max(0, nowMs - lastAttemptMs)
                         */
                        long waitedTimeMs = batch.waitedTimeMs(nowMs);

                        /**
                         *  batch.attempts() - 批次重试的次数
                         *  waitedTimeMs < retryBackoffMs  - 已等待的时间是否小于重试最大时间
                         *  如果是true，表明当前正在重试
                         */
                        ...
                        /**
                         *  如果正在重试，则等待时间为最大充实时间
                         *  否则，为linger的时间（每隔多长时间，即便batch没有满16KB，也会发送请求）
                         */
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        /**
                         *  TopicPartition对应的队列已经大于1，表明队列中已经至少有两个batch了
                         *  或者队列中第一个batch已经满了
                         */
                        boolean full = deque.size() > 1 || batch.isFull();
                        /**
                         * 已经等待的时间已经大于等于最大等待时间（linger或者retryBackoffMs）
                         */
                        boolean expired = waitedTimeMs >= timeToWaitMs;

                        /**
                         * 这几种情况都可以发送了
                         * full：当前批次已满，或者队列中已经有满了的批次
                         * expired：等待时间已经超时了
                         * exhausted:内存不够了
                         * closed:已经关闭了producer客户端
                         * flushInProgress:flush
                         */
                        boolean sendable = full || expired || exhausted || closed || flushInProgress();
                        if (sendable && !backingOff) {//满足可以发送的条件，并且当前并没有重试
                            readyNodes.add(leader);//当前可以发送的节点中，加入TopicPartition对应的Leader节点
                        } else {
                            //还需要等待的时间
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        ...
        if (d != null)
            return d;
        d = new ArrayDeque<>();
        //为什么要使用CopyOnWriteMap？
        //1.读多写少
        //2.map中的元素不会特别多，copy的时间短
        Deque<ProducerBatch> previous = this.batches.putIfAbsent(tp, d);
        if (previous == null)//if there was no mapping for the key
            return d;
        else//如果之前已经有了，此处处理的是并发编程，非常有value
            return previous;
    }
