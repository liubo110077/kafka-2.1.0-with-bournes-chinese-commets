...
public class Sender implements Runnable {
    ...
    void run(long now) {
        ...
        // 检查连接是否已经建立，注册读写事件
        // 把准备好的消息发送出去，并对响应进行处理
        client.poll(pollTimeout, now);
    }

    private long sendProducerData(long now) {

        //todo: 第一步：获得metadata中的Cluster信息，第一次没有任何数据
        //在第一次没有元数据的情况下，下边的代码都不会被执行。
        //当metadata已经获得之后，cluster中就会有kafka集群的元数据了
        Cluster cluster = metadata.fetch();
        // get the list of partitions with data ready to send
        /**
         * todo：第二步：获得TopicPartition对应的可发送消息的节点
         *             可能没有可用的leader节点
         */
        ...
        /**
         * todo：第三步：如果没有leader节点，需要重新获得元数据
         */
        ...
        /**
         * todo，步骤4：
         *      检查producer和kafka节点之间的是否能够建立连接，如果不能，则移除不能建立连接的节点
         */
        while (iter.hasNext()) {
            Node node = iter.next();
            /**
        ...
        /**
         * todo，步骤5：
         *       有可能要发送的partition有很多个，这些partition对应的leader可能在同一个节点上
         *       p0 -> broker:0
         *       p1 -> broker:0
         *       p2 -> broker:1
         *       p3 -> broker:2
         *       按照broker id进行分区，将同一个broker的partition分成一组
         *                key       value
         *       broker :  0         p0,p1
         *                 1         p2
         *                 2         p3
         *       第一次建立连接后，result.readyNodes中的元素已经都被移除了，accumulator.drain返回空Map
         *       if (nodes.isEmpty())
         *             return Collections.emptyMap();
         *       但是run()方法会一直循环，当第一次连接建立之后，之后就会将消息发送出去
         */
        ...
        /**
         *  todo: 步骤7
         *         将待发送的ProducerBatch封装成ClientRequest
         *         batches 的数据结构 Map<Integer, List<ProducerBatch>>
    ...
    private void completeBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response, long correlationId,
        ...
        if (error == Errors.MESSAGE_TOO_LARGE && batch.recordCount > 1 && !batch.isDone() &&
        ...
        } else if (error != Errors.NONE) { //如果不是消息过大的异常
            ...
            if (canRetry(batch, response, now)) {//是否可以重试
                log.warn(
                    ...
                    error);
                if (transactionManager == null) { //如果没有事物
                    //把批次重新添加到队列中
    ...
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        ...
        //生成ClientRequest，如果acks不等于0，则表示会等待服务端的响应
