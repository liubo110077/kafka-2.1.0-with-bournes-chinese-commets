...
public final class ProducerBatch {
    ...
    public FutureRecordMetadata tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers, Callback callback, long now) {
        if (!recordsBuilder.hasRoomFor(timestamp, key, value, headers)) {
            ...
            //将callback和future封装到Thunk，然后添加到thunks
    ...
    public boolean done(long baseOffset, long logAppendTime, RuntimeException exception) {
        ...
        //通过CAS操作，将finalState从null更新为FinalState.SUCCEEDED或者FinalState.FAILED
        if (this.finalState.compareAndSet(null, tryFinalState)) {
            //回调KafkaProducer.send的callback方法
    ...
    private void completeFutureAndFireCallbacks(long baseOffset, long logAppendTime, RuntimeException exception) {
        ...
        for (Thunk thunk : thunks) {
            try {
                if (exception == null) {
                    //TODO ??? 有一个疑问：thunk.future.value()在什么时后被赋值的？？？？？？答案是在本方法的produceFuture.set(baseOffset, logAppendTime, exception);
                    //TODO produceFuture.set(baseOffset, logAppendTime, exception);中的几个参数都是从response中获得的
                    RecordMetadata metadata = thunk.future.value();
                    if (thunk.callback != null)//没有异常的回调函数
